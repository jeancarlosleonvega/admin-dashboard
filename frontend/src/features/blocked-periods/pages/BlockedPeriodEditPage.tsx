...
export default function BlockedPeriodEditPage() {
  const { id } = useParams<{ id: string }>();
  const navigate = useNavigate();
  usePageHeader({ subtitle: 'Editar período bloqueado' });
  const queryClient = useQueryClient();

  const { data: period, isLoading, isError } = useQuery({
    queryKey: ['blocked-periods', id],
    queryFn: () => blockedPeriodsApi.getBlockedPeriod(id!),
    enabled: !!id,
  });
  ...
  useEffect(() => {
    if (period) {
      reset({
        sportTypeId: period.sportTypeId ?? null,
        venueId: period.venueId ?? null,
  ...
  const updateMutation = useMutation({
    mutationFn: (data: FormData) =>
      blockedPeriodsApi.updateBlockedPeriod(id!, {
        ...
        <form onSubmit={handleSubmit(onSubmit)}>
          ...
          <div className="px-6 pb-6 flex justify-end gap-3 pt-4 border-t border-gray-200">
            ...
            <button type="submit" disabled={updateMutation.isPending} className="btn-primary">
              {updateMutation.isPending ? <Spinner size="sm" className="text-white" /> : 'Guardar cambios'}
            </button>
          </div>
        </form>
      </div>
