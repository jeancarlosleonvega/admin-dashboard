...
export default function VenueScheduleCreatePage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!form.venueId || !form.name || !form.startDate || form.daysOfWeek.length === 0) {
    ...
    try {
      ...
      toast.success('Schedule creado exitosamente');
      navigate('/venue-schedules');
    } catch (err: unknown) {
      toast.error(err?.response?.data?.error?.message ?? 'Error al crear schedule');
    }
  };
    ...
    <div className="max-w-2xl">
      <form onSubmit={handleSubmit} className="card p-6 space-y-6">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">
              ID de Venue <span className="text-red-500">*</span>
            </label>
            <input
              type="text"
              value={form.venueId}
        ...
        <div className="flex justify-end gap-3">
          ...
          >
            {createSchedule.isPending && <Spinner size="sm" />}
            Crear Schedule
          </button>
        </div>
      </form>
