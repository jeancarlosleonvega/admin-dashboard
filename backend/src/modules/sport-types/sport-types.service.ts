...
export class SportTypesService {
  ...
  async delete(id: string): Promise<void> {
    ...
    const venueCount = await sportTypesRepository.countRelated(id);
    if (venueCount > 0) {
      throw new ValidationError(`No se puede eliminar: el deporte tiene ${venueCount} espacio${venueCount !== 1 ? 's' : ''} asociado${venueCount !== 1 ? 's' : ''}. Eliminá o reasigná los espacios primero.`);
    }
