  ...
  const onSubmit = async (data: FormData) => {
    try {
      await createVenue.mutateAsync({
        ...
        ...(data.intervalMinutes && data.intervalMinutes !== '' ? { intervalMinutes: Number(data.intervalMinutes) } : {}),
        ...(data.playersPerSlot && data.playersPerSlot !== '' ? { playersPerSlot: Number(data.playersPerSlot) } : {}),
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <input
                id="openTime"
                type="time"
                placeholder={selectedSportType?.defaultOpenTime}
                className="input"
                {...register('openTime')}
              />
            </div>
            <div>
              ...
              <input
                id="closeTime"
                type="time"
                placeholder={selectedSportType?.defaultCloseTime}
                className="input"
                {...register('closeTime')}
              />
            </div>
